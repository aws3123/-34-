...
public sealed class SensorSimulatorTests
{
    ...
    public void IsTemperatureStable_Requires_MultipleConsecutiveStableTicks()
    {
        ...
        // Each Preparing update at 749+ clamps to 750
        for (int i = 0; i < 5; i++)
            sim.Update(TestState.Preparing);

        bool stable = sim.IsTemperatureStable();

        // 749+ means it clamps to 750, so each tick is "in range"
        // stableTickCount should be > 3 after 4+ ticks
        Assert.True(stable);
    }

    [Fact]
    public void ElapsedSeconds_Increments_OnlyInRecording()
    {
        var settings = CreateSettings();
        var sim = new SensorSimulator(settings);

        // Preparing mode should not increment elapsed
        sim.Update(TestState.Preparing);
        Assert.Equal(0, sim.ElapsedSeconds);

        // Recording should increment
    ...
    public void StableCounter_Resets_WhenTemperatureOutOfRange()
    {
        ...
        // Build up stable ticks
        for (int i = 0; i < 5; i++)
            sim.Update(TestState.Preparing);

        Assert.True(sim.IsTemperatureStable());

        // Reset and start from cold — counter should reset
