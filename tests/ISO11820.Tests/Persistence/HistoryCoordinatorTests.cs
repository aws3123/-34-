...
public sealed class HistoryCoordinatorTests : IDisposable
{
    ...
    public void QueryTestTypes_Should_Filter_By_TestType()
    {
        using var connection = _dbHelper.CreateConnection();
        using var cmd = new SqliteCommand(
            "INSERT INTO testmaster (test_type, description, standard) VALUES ('Tensile', 'Tensile strength test', 'ISO 6892')",
            connection);
        cmd.ExecuteNonQuery();

        var tests = _coordinator.QueryTestTypes("Tensile");
        Assert.Single(tests);
        Assert.Equal("Tensile", tests[0].TestType);
        Assert.Equal("ISO 6892", tests[0].Standard);
    }
}