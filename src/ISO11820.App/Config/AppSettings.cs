...
public sealed class AppSettings
{
    ...
    public AppSettings ResolvePaths(string baseDirectory)
    {
        return new AppSettings
        {
            ...
            FileStorage = FileStorage.ResolvePaths(baseDirectory),
            Report = Report.ResolvePaths(baseDirectory),
            Hardware = Hardware,
        };
    }
}
...
{
    public string BaseDirectory { get; init; } = "TestData";
    public string TestDataDirectory { get; init; } = "TestData/Samples";

    public FileStorageSettings ResolvePaths(string baseDirectory)
    {
        return new FileStorageSettings
        {
            BaseDirectory = AppSettingsPathResolver.Resolve(baseDirectory, string.IsNullOrWhiteSpace(BaseDirectory) ? "TestData" : BaseDirectory),
            TestDataDirectory = AppSettingsPathResolver.Resolve(baseDirectory, string.IsNullOrWhiteSpace(TestDataDirectory) ? "TestData/Samples" : TestDataDirectory),
        };
    }
}
...
{
    public string OutputDirectory { get; init; } = "TestData/Reports";
    public bool EnablePdfExport { get; init; } = true;

    public ReportSettings ResolvePaths(string baseDirectory)
    {
        return new ReportSettings
        {
            OutputDirectory = AppSettingsPathResolver.Resolve(baseDirectory, string.IsNullOrWhiteSpace(OutputDirectory) ? "TestData/Reports" : OutputDirectory),
...
{
    public double ConstPower { get; init; } = 2048;
    public double PidTemperature { get; init; } = 750;
}
