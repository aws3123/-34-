...
public sealed class CalibrationCoordinator
{
    ...
    public CalibrationRecord? QueryRecordById(long id)
    {
        using var connection = _dbHelper.CreateConnection();
        var sql = "SELECT id, sensor_id, calibration_date, result_json, technician, notes, created_at FROM \"CalibrationRecords\" WHERE id = @id";
