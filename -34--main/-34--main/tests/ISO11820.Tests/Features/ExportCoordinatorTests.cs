...
public sealed class ExportCoordinatorTests : IDisposable
{
    ...
    public ExportCoordinatorTests()
    {
        ...
        _coordinator = new ExportCoordinator(_csvWriter);
    }
    ...
    [Fact]
    public void ExportToExcel_Should_Return_Success_As_Stub()
    {
        var request = new ExportRequest { ProductId = "P001", TestId = "T001" };
        var result = _coordinator.ExportToExcel(request);

        // 目前是预留功能，应返回成功状态但带有提示信息
        Assert.True(result.Success);
        Assert.NotNull(result.Message);
        Assert.Equal(ExportFormat.Excel, result.Format);
    }

    [Fact]
    public void ExportToExcel_Should_Return_Xlsx_Path()
    {
        var request = new ExportRequest { ProductId = "P001", TestId = "T001" };
        var result = _coordinator.ExportToExcel(request);
    ...
    [Fact]
    public void ExportToPdf_Should_Return_Success_As_Stub()
    {
        var request = new ExportRequest { ProductId = "P001", TestId = "T001" };
        ...
        Assert.True(result.Success);
        Assert.NotNull(result.Message);
        Assert.Equal(ExportFormat.Pdf, result.Format);
    }

    [Fact]
    public void ExportToPdf_Should_Return_Pdf_Path()
    {
        var request = new ExportRequest { ProductId = "P001", TestId = "T001" };
        var result = _coordinator.ExportToPdf(request);
    ...
    {
        Assert.Throws<ArgumentNullException>(() => new ExportCoordinator(null!));
    }
